...
class GetIp(Singleton):
    ...
    def get_ips(self):
        ...
        # httpx = [h[0:2] for h in self.result if h[2] == "HTTP, HTTPS" and self.judge_ip(h)]
        print "Http: ", len(http), "Https: ", len(https)  #, "Httpx: ", len(httpx)
        return {"http": http, "https": https}  # "httpx": httpx}
